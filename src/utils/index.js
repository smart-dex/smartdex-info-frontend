...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `${process.env.REACT_APP_EXCHANGE_URL}/#/pool/` +
      (remove ? `remove` : `add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `${process.env.REACT_APP_EXCHANGE_URL}/#/swap?inputCurrency=${token0Address}`
  } else {
    return `${process.env.REACT_APP_EXCHANGE_URL}/#/swap?inputCurrency=${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export const urls = {
  showTransaction: (tx) => `${process.env.REACT_APP_BSC_SCAN}/tx/${tx}/`,
  showAddress: (address) => `${process.env.REACT_APP_BSC_SCAN}/address/${address}/`,
  showToken: (address) => `${process.env.REACT_APP_BSC_SCAN}/token/${address}/`,
  showBlock: (block) => `${process.env.REACT_APP_BSC_SCAN}/block/${block}/`,
}
...
export const formattedValue = (number, usd = false, check = false) => {
  if (isNaN(number) || number === '' || number === undefined) {
    return usd ? '$0' : 0
  }
  let num = parseFloat(number)
  ...
  return check ? Number(parseFloat(num).toFixed(3)) : Number(parseFloat(num).toFixed(5)).toLocaleString()
}
